const table = {
  barang_keluar: $("#table-barang_keluar").DataTable({
    responsive: true,
    ajax: {
      url: origin + "/api/barang-keluar",
      dataSrc: "",
    },
    order: [
      [2, "asc"],
    ...
    case "delete":
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "DELETE",
            url: origin + "/api/barang-keluar/" + id,
            cache: false,
            success: (data) => {
              table.barang_keluar.ajax.reload();
              if (data.messages) {
                $.each(data.messages, function (icon, text) {
                  Toast.fire({
                    icon: icon,
...
$("form#form-edit").on("submit", function (e) {
  ...
  $.ajax({
    type: "POST",
    url: origin + "/api/barang-keluar/" + data.id,
    data: data,
    cache: false,
    success: (data) => {
      $(this)[0].reset();

const table = {
  pengadaan_karyawan: $("#table-pengadaan_karyawan").DataTable({
    responsive: true,
    ajax: {
      url: origin + "/api/pengadaan-karyawan",
      dataSrc: "",
    },
    order: [
      [2, "asc"],
    ...
    case "delete":
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "DELETE",
            url: origin + "/api/pengadaan-karyawan/" + id,
            cache: false,
            success: (data) => {
              table.pengadaan_karyawan.ajax.reload();
              if (data.messages) {
                $.each(data.messages, function (icon, text) {
                  Toast.fire({
                    icon: icon,
    ...
    case "acc":
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "POST",
            url: origin + "/api/pengadaan-karyawan/acc/" + id,
            cache: false,
            success: (data) => {
              table.pengadaan_karyawan.ajax.reload();
              if (data.messages) {
    ...
    case "tolak":
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "POST",
            url: origin + "/api/pengadaan-karyawan/tolak/" + id,
            cache: false,
            success: (data) => {
              table.pengadaan_karyawan.ajax.reload();
              if (data.messages) {
    ...
    case "selesai":
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: "POST",
            url: origin + "/api/pengadaan-karyawan/selesai/" + id,
            cache: false,
            success: (data) => {
              table.pengadaan_karyawan.ajax.reload();
              if (data.messages) {
...
$("form#form-edit").on("submit", function (e) {
  ...
  $.ajax({
    type: "POST",
    url: origin + "/api/pengadaan-karyawan/" + data.id,
    data: data,
    cache: false,
    success: (data) => {
      $(this)[0].reset();
...
$(document).ready(function () {
  loadBarangOptions();
  cloud
    .add(origin + "/api/pengadaan-karyawan", {
      name: "pengadaan_karyawan",
      callback: (data) => {
        table.pengadaan_karyawan.ajax.reload();
      },
    })
    .then((pengadaan_karyawan) => {});
  $(".preloader").slideUp();
});
